...
Keep changes on that boundary. Hearth does not implement inference kernels, vendor runtime
...
- `charts/hearth/crds/*.yaml` — synchronized by `make helm-crds`;
...
`charts/hearth/templates/rbac.yaml`, manager settings with both deployment definitions, and API
...
driver and device-plugin versions, runtime image, Hearth version, commands, and observed results.
...
- Keep controllers, builders, examples, tests, generated files, and `docs/crd-reference.md` aligned
  with API changes.
...
1. Update `charts/hearth/Chart.yaml` `version` and `appVersion`.
...
current kube-context, and never target development, staging, or production clusters. The standard
E2E suite manages its own cluster; the scale-to-zero suite expects its cluster and KEDA to exist.
...
- Preserve `serving.hearth.dev/*` and `app.kubernetes.io/*` naming and label contracts.
