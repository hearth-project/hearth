...
For a service that can reconcile in v0, set `spec.model.source.uri` and select a runtime with either
`spec.runtime.name` or `spec.runtime.selector.vendor`. Catalog-only models and an omitted runtime are
admitted by the alpha schema but reported as `Degraded` until those reserved paths are implemented.
...
| `spec.scaling.metric` | string | default `queueDepth`; enum `queueDepth`, `kvCacheUtil` | `queueDepth` drives scaling in v0. `kvCacheUtil` is reserved and rejected during reconciliation. |
...
| `spec.scaling.drainTimeout` | duration string | `2m` | Pre-stop wait for in-flight requests. Hearth widens the Pod termination grace period to this timeout plus a shutdown margin when needed. |
...
| `spec.cache.prewarm` | boolean | - | Creates a one-time Job that hydrates model weights into the persistent cache. |
| `spec.endpoint` | object | - | Client-facing endpoint behavior. |
| `spec.endpoint.openAICompatible` | boolean | `true` | Informational in v0; the gateway always exposes the OpenAI-compatible API. |
...
|---|---|---|
| `spec.family` | yes | Runtime family; v0 accepts only `vllm` and defaults to it. |
| `spec.vendor` | yes | Registered adapter key: `nvidia` or `ascend`. |
| `spec.priority` | no | Tie-breaker within one vendor; higher values win. Vendor order in an `LLMService` selector wins before priority. Equal top priorities are rejected as ambiguous; pin `spec.runtime.name`. |
| `spec.container.image` | yes | Serving image. It must expose an OpenAI-compatible API. |
...
| `spec.container.port` | yes | API port, also used for runtime metrics when available. `containerPort` must be between `1` and `65535`. |
...
| `spec.metrics` | no | Optional runtime metric metadata for external observability integrations. Hearth autoscaling uses the gateway queue instead. |
